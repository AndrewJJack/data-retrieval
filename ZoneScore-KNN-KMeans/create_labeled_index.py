...
def tokenize_insert(contents, doc_id, conn, doc_class):
        ...
        if token not in punctuation:
            ...
            if stemmed_word not in []:
                try:
                    ...
                    pass
                conn.execute('''INSERT INTO instances VALUES ("{}","{}","{}","{}")'''.format(stemmed_word, doc_id, position, doc_class)) 
...
def main():
    ...
    #opening index_directory and put the index there
    try: 
    ...
    for file in file_list:
        ...
        try:
            ...
            get_class = 0
            doc_class = ''
            for file_contents in  working_file.readlines():
                if get_class == 0:
                    doc_class = file_contents[:-1]
                    get_class += 1
                else:
                    tokenize_insert(file_contents, doc_id, conn, doc_class)
            working_file.close()
        except Exception as e:
